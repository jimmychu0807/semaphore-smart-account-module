...
contract SemaphoreAccountFactory {
  ...
  function createAccount(uint256 groupId, uint256 salt) public returns (SemaphoreAccount ret) {
    ...
    ret = SemaphoreAccount(
      payable(
        new ERC1967Proxy{salt: bytes32(salt)}(
          ...
          abi.encodeCall(SemaphoreAccount.initialize, (semaphoreAddress, verifierAdress, groupId))
        )
      )
    );
  }

  function getAddress(uint256 groupId, uint256 salt) public view returns (address) {
    return
