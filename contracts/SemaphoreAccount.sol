...
contract SemaphoreAccount is BaseAccount, UUPSUpgradeable, Initializable {
  ...
  event SemaphoreAccountInitialized(IEntryPoint indexed entryPoint, uint256 indexed groupId);
  ...
  function execute(address dest, uint256 value, bytes calldata func) external verifyGroup {
    _requireFromEntryPoint();
    _call(dest, value, func);
  }

  function executeBatch(address[] calldata dest, bytes[] calldata func) external verifyGroup {
    _requireFromEntryPoint();
  ...
  function _validateSignature(
    ...
    ) = abi.decode(userOp.signature, (uint256[8], uint256, uint256, uint256));
